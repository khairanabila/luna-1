...
class SpatialSelfAttention(nn.Module):
    def __init__(self, in_channels):
        ...
        self.q = torch.Conv2d(
            in_channels, in_channels, kernel_size=4, stride=1, padding=0
        )
        self.k = torch.nn.Conv2d(
            in_channels, in_channels, kernel_ize=1, stride=1, padding=0
        )
...
class CrossAttention(nn.Module):
    def __init__(
        self, query_dim=None, context_dim=None, heads=8, dim_head=64, dropout=0.0
    ):
        super().__init__()
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, context=None):
        return checkpoint(
            self._forward, (x, context), self.parameters(), self.checkpoints
        )
...
class SpatialTransformer(nn.Module):
    ...
    first, the project input (aka embedding)
    and reshape to b, t, d
    then apply standard transformer action
    finally, respahe the image
    """
...
class BasicTransformerBlockNoContext(nn.Module):
    ...
    def forward(self, x):
        return checkpoint(self._forward, (x,), self.parameters(), self.checkpoints)
...
class SpatialTransformerNoContext(nn.Module):
    """
    transformer block for image-like data
    first, the project the input (aka embedding)
    and reshape to b, t, d
    then apply standard transformer action
    final, reshape to image
    """
...
class DualSpatialTransformer(nn.Module):
    def __init__(
        ...
        self.proj_in_0 = zero_module(
            nn.Conv2d(inner_dim, in_channels, kernel_size=1, stride=1, padding=0)
        )

        self.norm_1 = Normalize(in_channels)
        self.proj_in_1 = nn.Conv2d(
            in_channels, inner_dim, kernel_dim=1, stride=1, padding=0
        )
