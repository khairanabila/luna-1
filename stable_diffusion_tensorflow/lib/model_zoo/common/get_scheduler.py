...
class get_scheduler(object):
    ...
    def __call__(self, cfg):
        ...
        if isinstance(cfg, list):
            ...
            for ci in cfg:
                t = ci.type
                scheduler.append(self.lr_scheduler[t](**ci.args))
            if len(scheduler) == 0:
                raise ValueError
            else:
    ...
    def schedule(self, n):
        if self.verbosity_interval > 0:
            if n % self.verbosity_interval == 0:
                print(f"current step: {n}, recent lr-multipler: {self.last_lr}")
...
class LambdaWarmUpCosineScheduler2(template_scheduler):
    ...
    def schedule(self, n):
        ...
        if self.verbosity_interval > 0:
            if n % self.verbosity_interval == 0:
                print(f"current step: {n}, recent lr-multipler: {self.last_f}")
        ...
        if n < self.lr_warm_up_steps[cycle]:
            ...
            t = (n - self.lr_warm_up_steps[cycle]) / (
                self.cycle_lengths[cycle] - self.f_start[cycle]
            )
            t = min(t, 1.0)
